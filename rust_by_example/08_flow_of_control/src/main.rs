...
// That is why comparing Fu::Bar == a fails below.
enum Fu {Bar}
...
fn main() {
    ...
    let color = Color::CMYK(20, 15, 15, 40);
    ...
    match value {
        ref r => println!("Got a reference to a value: {r:?} and its pointer address is {r:p}"),
    }
    ...
    match mut_value {
        mut n => {
            n += 20;
            println!("Then tried to add 20 without reference. `mut_value`: {n:?}");
        }
    }
    ...
    match foo {
        FooStruct { x: (1, b), y } => println!("First of x is 1, b = {},  y = {} ", b, y),

        // you can destructure structs and rename the variables,
        // the order is not important
        FooStruct { y: 2, x: i } => println!("y is 2, i = {:?}", i),

        // and you can also ignore some variables:
        FooStruct { y, .. } => println!("y = {}, we don't care about x", y),
        // this will give an error: pattern does not mention field `x`
        //Foo { y } => println!("y = {}", y),
    }

    let faa = FooStruct { x: (1, 2), y: 3 };

    // You do not need a match block to destructure structs:
    let FooStruct { x : x0, y: y0 } = faa;
    ...
    let bar = BarStruct { foo: faa };
    let BarStruct { foo: FooStruct { x: nested_x, y: nested_y } } = bar;
